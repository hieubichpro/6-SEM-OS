...
static struct workqueue_struct *mywq1, * mywq2;
...
static void mywq_fun_1(struct work_struct *work)
{
    ...
    if (mywork->code < 84)
        printk(KERN_INFO "+ mywq: mywq_fun_1: time = %llu, code = %s, code = %d\n", ktime_get(), ascii[mywork->code], mywork->code);
    else
        printk(KERN_INFO "+ mywq: mywq_fun_1: time = %llu, code = %d\n", ktime_get(), mywork->code);
...
static void mywq_fun_2(struct work_struct *work)
{
    ...
    msleep(1000);
    printk(KERN_INFO "+ mywq: mywq_fun_2: time = %llu, code = %d\n", ktime_get(), mywork->code);
}
...
static irqreturn_t my_irq_handler(int irq, void *dev_id)
{
    ...
    if (irq == IRQ_NO)
    {
        queue_work(mywq1, (struct work_struct *)work_1);
        queue_work(mywq2, (struct work_struct *)work_2);
        return IRQ_HANDLED;
    }
...
static int __init mywq_init(void)
{
    ...
    mywq1 = alloc_workqueue("%s", __WQ_LEGACY | WQ_MEM_RECLAIM, 1, "my_queue1");
    ...
    mywq2 = alloc_workqueue("%s", __WQ_LEGACY | WQ_MEM_RECLAIM, 1, "my_queue2");
...
static void __exit mywq_exit(void)
{
    ...
    flush_workqueue(mywq1);
    flush_workqueue(mywq2);
    destroy_workqueue(mywq1);
    destroy_workqueue(mywq2);
    free_irq(IRQ_NO, (void *)(my_irq_handler));
    kfree(work_1);
    kfree(work_2);
